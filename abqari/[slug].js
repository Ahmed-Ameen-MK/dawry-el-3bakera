// api/abqari/[slug].js
// Vercel Serverless Function — بديل عن index.php
// الرابط: https://dawry-el-3bakera.vercel.app/abqari/ahmed-ameen
...
function html(user, rank, slug, error) {
  const name    = esc(user?.name    ?? 'غير معروف');
  const email   = esc(user?.email   ?? '');
  const points  = parseInt(user?.points ?? 0);
  const country = esc(user?.country ?? '');
...
<meta property="og:description" content="${user ? `${name} - نقاط: ${points} | الترتيب: #${rank}` : ''}">
<meta property="og:url" content="https://dawry-el-3bakera.vercel.app/abqari/${encodeURIComponent(slug)}">
...
.back-btn{display:flex;align-items:center;gap:6px;background:var(--bg2);border:1px solid var(--line);border-radius:20px;padding:6px 14px;font-family:'El Messiri',sans-serif;font-size:13px;font-weight:500;color:var(--ink2);text-decoration:none;transition:all .15s}
...
.rank-badge{position:absolute;bottom:0;left:50%;transform:translateX(-50%);background:${color};color:#fff;border-radius:20px;padding:3px 10px;font-size:12px;font-weight:700;white-space:nowrap;border:2px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.15)}
.profile-name{font-size:26px;font-weight:700;letter-spacing:-.5px;margin-bottom:6px}
...
.share-btn{width:100%;padding:12px;background:var(--blue);color:#fff;border:none;border-radius:10px;font-family:'El Messiri',sans-serif;font-size:15px;font-weight:600;cursor:pointer;transition:all .15s;margin-top:8px;display:flex;align-items:center;justify-content:center;gap:8px}
...
<div class="profile-wrap">
  <div class="profile-card">
    ...
    <div class="stats-row">
      <div class="stat-box">
        <div class="stat-val">${points.toLocaleString()}</div>
    ...
    <div class="info-row">
      ...
      <span style="font-size:12px;color:var(--ink3);word-break:break-all">
        dawry-el-3bakera.vercel.app/abqari/${esc(slug)}
      </span>
    </div>
...
function shareProfile() {
  const url = 'https://dawry-el-3bakera.vercel.app/abqari/${encodeURIComponent(slug)}';
...
export default async function handler(req, res) {
  ...
  if (!slug) {
    ...
    return res.status(400).send(html(null, null, '', 'رابط غير صحيح'));
  }
  ...
  try {
    ...
    if (!userData || !userData[0]) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.status(404).send(html(null, null, slug, 'لم يُعثر على هذا الحساب'));
    }
    ...
    const rank = (Array.isArray(rankData) ? rankData.length : 0) + 1;

    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.status(200).send(html(user, rank, slug, ''));

  } catch (err) {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.status(500).send(html(null, null, slug, 'خطأ في الخادم'));
  }
}
